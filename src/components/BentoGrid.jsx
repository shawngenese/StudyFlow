  ...
  const tileMap={
    hero: (
      <Tile key="hero" id="hero" className="tile-hero" isEditing={isEditing}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'6px'}}>
          <h3 style={{fontSize:'15px'}}>Today focus</h3>
          <span className="muted">{visibleTasks.filter(t=>!t.completed).length} tasks · {new Date().toLocaleDateString(undefined,{weekday:'short', month:'short', day:'numeric'})}</span>
        </div>
        {heroTasks.length===0 ? <p className="muted">No tasks — add one below or in List.</p> : (
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleHeroTaskDragEnd}>
            <SortableContext items={heroTasks.map(t=>t.id)} strategy={rectSortingStrategy}>
              <ul style={{listStyle:'none',margin:0,padding:0,display:'flex',flexDirection:'column',gap:'6px'}}>
                {heroTasks.map(t=>(
                  <li key={t.id} style={{display:'flex',alignItems:'center',gap:'8px',padding:'8px 10px',background:'var(--bg-inset)',borderRadius:'10px'}}>
                    <input type="checkbox" checked={t.completed} onChange={()=>dispatch({type:'task/toggle',id:t.id})} style={{accentColor:'var(--success)'}} />
                    <span onClick={()=>onSelect(t.id)} style={{flex:1,cursor:'pointer',fontWeight:500, textDecoration: t.completed?'line-through':'none'}}>{t.text.slice(0,60)}</span>
                    {heroReorder && <span style={{color:'var(--text-muted)',fontSize:'10px'}}>drag</span>}
                  </li>
                ))}
              </ul>
            </SortableContext>
    ...
    stats: (
      <Tile key="stats" id="stats" className="tile-stats" isEditing={isEditing}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
          <div><div className="muted" style={{fontSize:'10px',textTransform:'uppercase'}}>Today</div><div style={{fontSize:'20px',fontWeight:700}}>{data.tasks.filter(x=>!x.completed&&x.dueDate===todayKey()).length}</div></div>
          <div><div className="muted" style={{fontSize:'10px',textTransform:'uppercase'}}>Overdue</div><div style={{fontSize:'20px',fontWeight:700,color:stats.overdue?'var(--danger)':'inherit'}}>{stats.overdue}</div></div>
          <div><div className="muted" style={{fontSize:'10px',textTransform:'uppercase'}}>Focus</div><div style={{fontSize:'20px',fontWeight:700}}>{Math.round(stats.focusMins)}m</div></div>
          <div><div className="muted" style={{fontSize:'10px',textTransform:'uppercase'}}>Streak</div><div style={{fontSize:'20px',fontWeight:700}}>{stats.avgStreak}d</div></div>
        </div>
      </Tile>
    ),
    focus: (
        ...
        {(data.habits||[]).length===0 ? <p className="muted">No habits yet</p> : (
          <div style={{display:'flex',flexDirection:'column',gap:'6px',marginTop:'8px'}}>
            {(data.habits||[]).slice(0,3).map(h=>(
              <div key={h.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'6px 8px',background:'var(--bg-inset)',borderRadius:'8px'}}>
                <span style={{fontSize:'13px',fontWeight:500}}>{h.icon} {h.name}</span>
                <button type="button" className={h.completions?.[todayKey()] ? 'btn' : 'todo-add-btn'} style={{fontSize:'11px',padding:'4px 10px'}} onClick={()=>dispatch({type:'habit/toggle', id:h.id, date:todayKey()})}>{h.completions?.[todayKey()]?'Done':'Do'}</button>
              </div>
            ))}
          </div>
        )}
    ...
    cal: (
      <Tile key="cal" id="cal" className="tile-cal" isEditing={isEditing}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'6px',marginTop:'10px',textAlign:'center'}}>
          {[...Array(7)].map((_,i)=>{
            ...
            return <div key={k} style={{padding:'8px 4px',borderRadius:'10px',background:isToday?'var(--accent-soft)': 'var(--bg-inset)', border: isToday?'1px solid var(--accent-border)':'1px solid transparent'}}>
              <div style={{fontSize:'10px',color:'var(--text-muted)'}}>{d.toLocaleDateString(undefined,{weekday:'short'})}</div>
        ...
        {Object.keys(stats.byCourse).length===0 ? <p className="muted">No active tasks</p> : (
          <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
            {Object.entries(stats.byCourse).slice(0,4).map(([pid,c])=>{
              ...
              return <div key={pid} style={{display:'flex',alignItems:'center',gap:'8px'}}><span style={{width:'90px',fontSize:'12px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{proj?.name||pid}</span><div style={{flex:1,height:'6px',background:'var(--bg-inset)',borderRadius:'999px',overflow:'hidden'}}><div style={{width:`${(c/max)*100}%`,height:'100%',background:'var(--accent)'}}/></div><span style={{fontSize:'11px',color:'var(--text-muted)'}}>{c}</span></div>
            })}
          </div>
        )}
    ...
    <div>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleTileDragEnd}>
        <SortableContext items={order} strategy={rectSortingStrategy}>
          <div className={`bento bento-dashboard ${isEditing?'is-editing':''}`}>
            {orderedTiles}
          </div>
        </SortableContext>
      </DndContext>
